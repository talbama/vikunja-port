...
| Piece | File | Notes |
|---|---|---|
| Share settings modal | `frontend/src/views/project/settings/ProjectSettingsShare.vue` | Loads the project with legacy `ProjectService`, `userIsAdmin = maxPermission === PERMISSIONS.ADMIN`, renders `LinkSharing` (if `configStore.linkSharingEnabled && userIsAdmin`) and `UserTeam` twice (`shareType="user"` / `"team"`) |
...
| Directive | `frontend/src/directives/shortcut.ts` → `v-shortcut="SHORTCUTS.navigation.overview"` | registered in `main.ts`; used in `components/home/Navigation.vue`, `MenuButton.vue`, `ContentAuth.vue`, `views/tasks/TaskDetailView.vue` |
