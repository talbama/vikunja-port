...
| `components/gantt/*` | `GanttChartBody`, `GanttRow`, `GanttRowBars` (bar geometry, resize handles, drag preview), `GanttTimelineHeader` (uses `useGlobalNow`), `GanttVerticalGridLines`, `GanttRelationArrows`, `primitives/*` (headless bar/row/chart). `frontend/src/composables/useGanttBar.ts` (not under `components/gantt/`) holds `GanttBarModel` and keyboard resize (`changeSize`). |
