# Agent metrics — 2 runs, 73 events
...
|---|---|---|---|
| time_to_triage | median=47.18, p75=47.18 | 1 | run.start → first triage comment |
...
| autonomous_resolution_rate | 0.00 | 2 | over all runs; merged as-is = github.pr_merged.diff_changed_since_agent == false |

## Quality

| metric | value | n | notes/assumptions |
|---|---|---|---|
| reproduction_rate | 0.50 | 2 | repro.result.failed_on_main == true, over runs |
...
| metric | value | n | notes/assumptions |
|---|---|---|---|
| cost_per_run | 0.24 | 2 | mean of Σ phase.end.cost_usd per run |
| cost_per_resolved_bug | null | 0 | total cost of all runs (including failures) / merged PRs |
| budget_kill_rate | 0.00 | 2 | runs with budget.kill, over runs |
...
|---|---|---|---|---|---|---|---|---|
| r-4-0c8434 | 2026-09-17 | 0 | f47171bd | 16 | 98 | 45.1 | +0 | +0.0 |
...
quiet  budget_ceiling_wrong: budget_kill_rate > 0.2 (value=0.00, n=2, min_n=10)
```

_Generated 2026-09-17T14:18:37Z by run 35232671093._
